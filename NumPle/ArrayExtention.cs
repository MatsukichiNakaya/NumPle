...
namespace Project.Extention
{
    ...
    public static class ArrayExtention
    {
        ...
        public static TResult Minimum<T, TResult>(this IEnumerable<T> src,
            ...
            => Enumerable.Min(Enumerable.Select(src, selector));
        ...
        private static Int32[] GetRankIndex<T>(IEnumerable<T> src, Int32 count, Boolean sortType)
        {
            // 0 の場合は引き出す必要がないためそのまま返す 
            if (count == 0) { return new Int32[count]; }
            return (sortType ?
                    // 昇順(true)または降順(false)でデータの並べ替え
                    src.IndexOfAscSort() : src.IndexOfDescSort()
                    // 必要な数だけ取り出す
                    ).Take(count).ToArray();
        }
        #endregion
